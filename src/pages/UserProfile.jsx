...
export const UserProfile = () => {
  ...
  const user = auth?.user;
    ...
    const handleUpdateProfileSubmit = (e) => {
        ...
        dispatch(updateProfile({ name: name.trim(), bio: bio.trim(), avatar }));
        ...
        <div className="row g-4">
          ...
          <div className="col-12 col-lg-4">
            <div className="card card-custom p-4 text-center">
              <img src={auth?.user?.avatar} alt={auth?.user?.name} className="rounded-circle border mx-auto mb-3" width="110" height="110" style={{ objectFit: 'cover' }}/>
            ...
            <div className="card card-custom p-4">
              ...
              {activeSubTab === 'settings' && (<div className="animate-fade-in">
                  ...
                  <form onSubmit={handleUpdateProfileSubmit}>
                    ...
                    <div className="mb-3">
                      <label className="form-label small fw-bold d-block">Select Avatar Picture</label>
                      <div className="d-flex flex-wrap gap-2 mt-1">
                        {AVATARS.map((avUrl, index) => (<img key={index} src={avUrl} alt={`Avatar Option ${index}`} onClick={() => setAvatar(avUrl)} className={`rounded-circle border p-1`} width="52" height="52" style={{
                    cursor: 'pointer',
                    objectFit: 'cover',
                    borderColor: avatar === avUrl ? 'var(--primary-teal)' : '#dee2e6',
                    borderWidth: avatar === avUrl ? '3px' : '1px',
                }}/>))}
                      </div>
                    </div>
